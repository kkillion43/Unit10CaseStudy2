...
*AIC AICc BIC are very similar however the mean of the residuals within the Acutal data under holt winters additive model have some larger values.
...
*The multiplicative Holt Winters seems to be the slight favor here with its errors remaining lower compared to the Holts additive model.
...
```r
p_dictionPlan <- predict(linFit_ActPlan1, wugePlan, interval='predict')

plot(p_dictionPlan[,1]/1000, main = 'Fit with Acutal Seasonals and 95% Confidence Intervals')
lines(p_dictionPlan[,2]/1000, col='red')
lines(p_dictionPlan[,3]/1000, col='green')
lines(p_dictionPlan[,1]/1000, col='blue')
legend('topleft', legend=c("fit", "lwr", 'upr'),lty=1, cex=0.8, col = c('blue','red','green'))
```

![](MakeFile_files/figure-html/unnamed-chunk-22-1.png)<!-- -->
<hr>

*When observing the linear regression, the estimates for the model intercept is 4.019e+05 as opposed to the -1.343e+04 within the second regression which utilizes Planned Seasonals.*

*The other thing we thought was interesting was the NationalHolidays were right on Alpha within the Actual Seasonals, but conversely, Planned seasonals carried a high acceptance P value of __.7475__*
